        ...
        while True: 
            try: 
                ...
                while self.len > 0:
                    ...
                    self.len -= 1
                    print("packet has been serviced")
                    print("the serviced packet: " + str(packet.identifier))
                    print()
...
class Arrival_Process(object): 
    ...
    def run(self):
        ...
        while True:
            ...
            print("a new packet has been added")
            print("the packet number: " + str(self.packet_number))
            print()

            self.server_process.queue.append(new_packet)

            print("the queue size is now: " + str(len(self.server_process.queue)))
            #here is where we check whether we will have to retransmit from the different hosts
            #depending on if two hosts collided at the same time
